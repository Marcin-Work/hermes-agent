...
class TestSubdirectoryHintTracker:
    ...
    def test_seed_timeout_skips_slow_working_dir_hint(self, project, monkeypatch, caplog):
        """Constructor seeding must use the bounded read too: a slow CWD hint
        file (e.g. an evicted iCloud/FileProvider placeholder) cannot stall
        agent startup."""
        import sys
        ...
        def slow_read_text(self, *args, **kwargs):
            if self.name.lower() == "agents.md" and self.parent == root:
                time.sleep(0.6)
            return original_read_text(self, *args, **kwargs)
        ...
        assert elapsed < 0.4, f"constructor seeding blocked for {elapsed:.2f}s"
        # A timed-out CWD hint behaves like a missing one: nothing seeded.
