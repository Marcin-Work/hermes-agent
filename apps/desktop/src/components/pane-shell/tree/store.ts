...
import { resolveTabStripVisible } from './renderer/strip-visibility'
import { paneChrome, rootChildSide } from './renderer/track-model'
...
export function tabStripVisibleForGroup(group: GroupNode): boolean {
  ...
  return resolveTabStripVisible({
    headerVeto: paneChrome(paneFor(group.active)).headerVeto,
    mode: effectiveTabStripMode(group.tabStrip),
    shown: shown.map(id => ({
      collapsePane: isCollapsePane(id),
      placement: paneChrome(paneFor(id)).placement,
      uncloseable: paneChrome(paneFor(id)).uncloseable
    }))
  })
}
